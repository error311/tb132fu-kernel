...
static BLOCKING_NOTIFIER_HEAD(ctn730_notifier_chain);

int is_stylus_attached(void){
	return ctn730 ? ctn730->pen_info.attached : 0;
}
EXPORT_SYMBOL_GPL(is_stylus_attached);
int ctn730_notifier_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ctn730_notifier_chain, nb);
}
EXPORT_SYMBOL_GPL(ctn730_notifier_register);

int ctn730_notifier_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&ctn730_notifier_chain, nb);
}
...
{
	(void)ctn730_dev;
	return blocking_notifier_call_chain(&ctn730_notifier_chain, val, v);
}
			...
			switch(instruction){
				case HOST_RESET:
					if(data[0] == 0){
						...
						//desc:fix tp bug
						ctn730_err("call ctn730_notifier_call_chain()");
							...
							struct ctn730_dev *ctn730_dev)
{
	...
	val = of_get_named_gpio_flags(node, "boost-5v-gpios", 0, &flags);
	if (val >= 0) {
		ctn730_dev->boost_gpio = val;
	}
	else {
		dev_err(dev, "BOOST GPIO error getting from OF node\n");
		return val;
	}
...
static ssize_t hard_rst_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int enable;
	struct ctn730_dev *ctn730_dev = container_of(dev_get_drvdata(dev),
	...
	ctn730_err("enable=%d\n",enable);
	if(enable){
		if (!gpio_get_value(ctn730_dev->rst_gpio))
		ctn730_hard_reset(ctn730_dev);
	}else{
		gpio_set_value_cansleep(ctn730_dev->rst_gpio, 0);
		ctn730_dev->status = status_unkown;
		ctn730_dev->ext_cmd = 0;
		ctn730_err("status %s\n",status_text[ctn730_dev->status]);
		schedule_delayed_work(&ctn730_dev->check_static_charge_work, msecs_to_jiffies(DETECT_CHECK_DELAY_TIME));
	}
...
static ssize_t hard_rst_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ctn730_dev *ctn730_dev = container_of(dev_get_drvdata(dev),
			struct ctn730_dev, ctn730_device);
	...
	return sprintf(buf, "%d\n",gpio_get_value(ctn730_dev->rst_gpio));
}
...
static DEVICE_ATTR_RW(hard_rst);
...

{
	...
	ret = gpio_request(ctn730_dev->boost_gpio, "nfc_boost_5v");
	if (ret){
		ctn730_err("not able to get GPIO nfc_boost_5v\n");
		ret=  -ENODEV;
		goto exit5;
	}
	...
	ctn730_dbg("request boost_gpio %d\n", ctn730_dev->boost_gpio);
	...
	ret = gpio_direction_output(ctn730_dev->boost_gpio, 1);
	if (ret < 0) {
		ctn730_err("not able to set boost_gpio as output\n");
		goto exit5;
	}
	...
	ctn730_enable(ctn730_dev);
	ctn730_dev->report_info = 1;
	ctn730_hard_reset(ctn730_dev);//open ctn730
	ctn730_err("success\n");
	
	return 0;

exit5:
	gpio_free(ctn730_dev->boost_gpio);
exit4:

...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
    ...
    if (apdo_idx != -1){
		...
		pdpm->apdo_selected_pdo = apdo_idx;
				     ...
				     unsigned long event, void *data)
{
	...
	usbpd_check_usb_psy(__pdpm);
	pr_err("__pdpm->usb_psy:[%d]\n",__pdpm->usb_psy);
	if (__pdpm->usb_psy) {
		pr_err("power_supply change\n");
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	if (!ret)
		pdpm->cp.ibat_curr = val.intval/1000;//uA->mA
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	val.intval = pdpm->pps_supported;

	power_supply_set_property(pdpm->cp_psy,
			POWER_SUPPLY_PROP_SC_PPS_SUPPORTED, &val);
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	int step_bat_reg = 0;
	int ibus_total = 0;
	int vbat_limit = BATT_MAX_CHG_VOLT;
	...
	/* set icl according to temp */
	if (g_sw_jeita_sm == TEMP_T1_TO_T2){
		ibus_limit = 1200;
		//printk("%s:set ibus_limit:[%d]\n",__func__,ibus_limit);
	}
	else if (g_sw_jeita_sm == TEMP_T3_TO_T4){
		ibus_limit = 2000;
		vbat_limit = 4200;
		//printk("%s:set vbat_limit:[%d],ibus_limit:[%d]\n",__func__,vbat_limit,ibus_limit);
	}else if (g_sw_jeita_sm == TEMP_ABOVE_T4 || g_sw_jeita_sm == TEMP_BELOW_T0){
		usbpd_pm_enable_cp(pdpm, false);
		//printk("%s:jeita_sm:[%d],battery temp over high or too low,disable charger.\n",__func__,g_sw_jeita_sm);
	}

	/* battery voltage loop*/
	if (pdpm->cp.vbat_volt > vbat_limit)
		step_vbat = -pm_config.fc2_steps;
	else if (pdpm->cp.vbat_volt < vbat_limit - 7)
		step_vbat = pm_config.fc2_steps;;


	/* battery charge current loop*/
	if (pdpm->cp.ibat_curr < (ibus_limit*2))
		step_ibat = pm_config.fc2_steps;
	else if (pdpm->cp.ibat_curr > ((ibus_limit*2) + 100))
		step_ibat = -pm_config.fc2_steps;
	...
	if (pdpm->cp.bat_therm_fault ) { /* battery overheat, stop charge*/
		...
		pr_info("bat_ocp_fault:%d, bus_ocp_fault:%d, bat_ovp_fault:%d, \
	...
	if (pdpm->request_voltage > pdpm->apdo_max_volt - 800)
		pdpm->request_voltage = pdpm->apdo_max_volt - 800;
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (pdpm->cp.vbus_error_low || pdpm->cp.vbus_volt < pdpm->cp.vbat_volt * 207/100) {
			...
			pr_err("request_voltage:%d, request_current:%d\n",
					pdpm->request_voltage, pdpm->request_current);
		} else if (pdpm->cp.vbus_error_high || pdpm->cp.vbus_volt > pdpm->cp.vbat_volt * 211/100) {
			tune_vbus_retry++;
			pdpm->request_voltage -= 20;
			usbpd_select_pdo(pdpm,pdpm->request_voltage,
						 pdpm->request_current);
		...
		if (tune_vbus_retry > 25) {
			pr_info("Failed to tune adapter volt into valid range, \
	...
	case PD_PM_STATE_FC2_ENTRY_3:
		...
		if (!pdpm->cp.charge_enabled &&
			!(g_sw_jeita_sm == TEMP_ABOVE_T4 || g_sw_jeita_sm == TEMP_BELOW_T0)) {
...
static void usbpd_pm_workfunc(struct work_struct *work)
{
	...
	usbpd_pm_update_sw_status(__pdpm);
	usbpd_pm_update_cp_status(__pdpm);
	usbpd_pm_update_cp_sec_status(__pdpm);
	
	if (!usbpd_pm_sm(__pdpm) && __pdpm->pd_active)
		schedule_delayed_work(&__pdpm->pm_work,
				msecs_to_jiffies(PM_WORK_RUN_INTERVAL));
}

bool g_is_pps_ta(void){
	union power_supply_propval val = {0,};
	power_supply_get_property(__pdpm->cp_psy,POWER_SUPPLY_PROP_SC_PPS_SUPPORTED, &val);
	if(val.intval)
		return true;
	else
		return false;
...
static void usbpd_pm_disconnect(struct usbpd_pm *pdpm)
{
	...
	val.intval = false;
	power_supply_set_property(pdpm->cp_psy,POWER_SUPPLY_PROP_SC_PPS_SUPPORTED, &val);
    usbpd_pm_move_state(pdpm, PD_PM_STATE_ENTRY);
}
...
static void usb_psy_change_work(struct work_struct *work)
{
	...
	pr_err("[SC manager] >> pd_active %d,  value %d\n",
			pdpm->pd_active, value);

	if (!pdpm->pd_active && value)
		usbpd_pd_contact(pdpm, true);
	else if (pdpm->pd_active && !value)
...
static int usbpd_register_tcpc_notifier(struct usbpd_pm *pdpm)
{
	int ret;

	if (pdpm->tcp_nb_registered)
		return 0;
	...
	pdpm->tcp_nb_registered = true;
	return 0;
}

static void usbpd_tcpc_register_work(struct work_struct *work)
{
	struct usbpd_pm *pdpm = container_of(to_delayed_work(work),
					struct usbpd_pm, tcpc_register_work);

	if (usbpd_register_tcpc_notifier(pdpm) < 0)
		schedule_delayed_work(&pdpm->tcpc_register_work,
...
static int __init usbpd_pm_init(void)
{
	...
	INIT_DELAYED_WORK(&pdpm->tcpc_register_work,
			  usbpd_tcpc_register_work);
	
	/* register tcp notifier callback */
	pdpm->tcp_nb.notifier_call = pca_pps_tcp_notifier_call;
	ret = usbpd_register_tcpc_notifier(pdpm);
	if (ret < 0)
		schedule_delayed_work(&pdpm->tcpc_register_work,
				      msecs_to_jiffies(1000));
...
static void __exit usbpd_pm_exit(void)
{
	...
	cancel_delayed_work_sync(&__pdpm->tcpc_register_work);
	if (__pdpm->tcp_nb_registered)
		unregister_tcp_dev_notifier(__pdpm->tcpc, &__pdpm->tcp_nb,
					    TCP_NOTIFY_TYPE_USB);
